...
struct CdfIntSquareRootProcessBridgeWithDeriv
{
    ...
    boost::math::tuple<double, double> operator()(double const& x) {
        ...
        double z_b1 = 4 * kappa * sqrt(vu * vt) * exp(-0.5 * kappa * (t - u)) / ((sigma * sigma) * (1 - exp(-kappa * (t - u))));
        Complex b1 = besseli(z_b1, 0.5*delta-1);
        ...
        double scale = exp(abs(zc) - abs(z_b1));
        double cc = (h*x/PI)+(2/PI)*sin(h*k*x)*(b2*t1*t2).real()/(b1.real()*k)*scale;
        double dd = (h/PI)+(2/PI)*h*cos(h*k*x)*(b2*t1*t2).real()/(b1.real())*scale; // deriv wrt x        
        double tv = (b2*t1*t2).real()/b1.real()*scale;
        ...
        while (std::abs(tv)/k >= PI*eps/2) {
            ...
            tv = (b2*t1*t2).real()/b1.real()*scale;
...
Complex firstMomentIntegralSquareRootProcess(double kappa, double theta, double sigma, double u, double vu, double t, double vt) {
    ...
    double z_t2 = 4 * kappa * exp(-(kappa * (t - u)) / 2.) * pow(sigma, -2) * pow(1 - exp(kappa * (-t + u)), -1) * sqrt(vt) * sqrt(vu);
    Complex t2 = besseli(z_t2,-2 + d/2.) + besseli(z_t2, d/2.);
    
    Complex t3 = Complex(2,0)*besseli(z_t2,-1 + d/2.);
...
Complex secondMomentIntegralSquareRootProcess(double kappa, double theta, double sigma, double u, double vu, double t, double vt) {
    double temp1 = (4 * exp(-((t - u) * kappa) / 2.) * sqrt(vt * vu) * kappa) / ((sigma * sigma) * (1 - exp(-((t - u) * kappa))));
    double temp2 = (4 * kappa * exp(-(kappa * (t - u)) / 2.) * sqrt(vt * vu)) / ((sigma * sigma) * (1 - exp(-(kappa * (t - u)))));
    double scale = exp(abs(temp1) - abs(temp2));
...
double test(double p, double kappa, double theta, double sigma, double u, double vu, double t, double vt, double x) {
...
std::tuple<double,double,double> quantileHestonProcess(double p1, double p2, double p3, double r, double rho, double kappa, double theta, double sigma, double u, double vu, double Su, double t) {
    ...
    return std::make_tuple(Su * exp(m + s * qnorm(p3)), vt, intv);
}
